...
# for m in mappings:
#     left = str(m).replace('\\x01','1').replace('\\x00','0').replace("'","").replace("b","")
#     for n in mappings[m]:
#         print(left, ''.join([ str(a) for a in (map(int, n))]))
...
for _ in range(5):
    grid = find_next(grid)
    # for g in grid:
    #     print(''.join([ str(a) for a in (map(int, g))]))
    print(grid.sum())
print(grid.sum())
