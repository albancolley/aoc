...
class AocBase(metaclass=ABCMeta):
    ...
    def run(self, part1_path: str, part2_path: str) -> []:
        ...
        self.display(p1, "Part 1")
        p2 = self.run_part_2(part2_path)
        self.display(p2, "Part 2")
        results = {"Part 1": p1, "Part 2": p2}
        failed = False
    ...
    def display(self, results, part):
        ...
        for result in results:
            ...
            if result[1]:
                star = ' '
            print(f'{star}{result[0]:20s} {str(result[2])[0:30]:30s} '
                  f'{str(result[3])[0:30]:30s} {result[6]:010.6f}'
                  f'  {result[4]:010.6f}  {result[5]:010.6f}')
