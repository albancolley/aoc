...
class AocBase(metaclass=ABCMeta):
    ...
    def display(self, result):
        ...
        if result.success:
            star = ' '
        expected_width = max(30, len(result.expected), len(result.actual))
        if expected_width > 30:
            print(f'{star}{result.filename:20s} {"":{30}s} '
                  f'{"":{30}s} {result.overall_time:010.6f}'
            ...
            print(f'{star}{result.filename:20s} {result.expected:{30}s} '
                  f'{result.actual:{30}s} {result.overall_time:010.6f}'
                  f'  {result.load_time:010.6f}  {result.calc_time:010.6f}')
